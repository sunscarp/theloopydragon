...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: orderData, error: fetchError } = await supabase
      .from("Orders")
      .select("seller_id, Email, Name, order_id, \"Total Price\", \"Shipping Cost\"")
    ...
    if (updateError) {
      console.error("Manual reject update error:", updateError);
    ...
    return NextResponse.json({
      success: true,
      message: "Order manually rejected, penalty applied, customer notified",
      penaltyAmount,
    });
  } catch (error) {
