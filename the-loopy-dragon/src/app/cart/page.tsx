...
export default function CartPage() {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === 'SIGNED_IN' && session?.user) {
        ...
        if (shouldGoToCheckout === "true") {
          console.log("Auth event: redirecting to checkout");
  ...
  // Function to handle Google Sign-In using Supabase
  const handleGoogleSignIn = async () => {
    // Use the login route for consistent OAuth flow
    const loginUrl = `/login?redirect=/cart&checkout=true`;
    window.location.replace(loginUrl);
  };

  const handleProceedToCheckout = () => {
    if (!user) {
      // Directly trigger Google sign-in instead of navigating to /login
      handleGoogleSignIn();
      return;
    }
