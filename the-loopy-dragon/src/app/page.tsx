  ...
  useEffect(() => {
    ...
    async function fetchTopPicks() {
      const { data, error } = await supabase
        .from('Inventory')
        .select('Product, Price, ImageUrl1, status')
        .in('Product', [
          "White Earrings",
          "Cat Ears White",
          "Sunflower Claw Clip",
      ...
      if (!error && data) {
        const active = data.filter(p => p.status !== "deactivated");
        ...
        const sorted = order
          .map(name => active.find((item: any) => item.Product === name))
        ...
        >
          <div className="max-w-7xl mx-auto px-6 sm:px-8 md:px-10 lg:px-12">
            ...
            <div className="text-center mb-12">
              ...
              >
                Our newest handmade pieces! Add to cart before they disappear.
              </p>
            </div>
      ...
      {faqs.map((faq, idx) => (
        <div className="mb-4" key={idx}>
          <div className="flex justify-between items-center py-2 gap-6">
            <h3 
              className="text-black flex-1 cursor-pointer" // Added cursor-pointer
              style={{
                fontFamily: 'Montserrat, sans-serif',
              ...
              onClick={() => setOpenFaq(openFaq === idx ? null : idx)} // Make question clickable
            >
              {faq.question}
            </h3>
            <button
