...
export default function OrdersPage() {
  ...
  const updateTrackingID = async (orderId: string, newTracking: string) => {
    ...
    try {
      ...
      if (error) {
        throw new Error("Failed to update tracking ID: " + error.message);
      }

      setOrders(prev => prev.map(order =>
        order.order_id === orderId
          ? { ...order, Tracking_ID: newTracking }
          : order
      ));
      ...
      alert(err instanceof Error ? err.message : "Failed to update tracking ID");
    } finally {
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredAndSortedOrders.map((order) => {
                  ...
                  const isEditingTracking = editingTracking === order.order_id;
                  const isUpdatingTracking = updatingTracking === order.order_id;
                    ...
                    <tr key={order.order_id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4">
                        <div className="space-y-1">
                          ...
                          {isEditingTracking ? (
                            <div className="flex items-center space-x-2 mt-1">
                              <input
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === 'Enter' && customTracking.trim()) {
                                    updateTrackingID(order.order_id, customTracking);
                                  }
                                }}
                            ...
                            order.Tracking_ID && (
                              <div className="flex items-center space-x-2 mt-1">
                                <span className="text-xs font-semibold text-green-700 dark:text-green-300">Tracking ID:</span>
                                <span className="text-xs text-gray-900 dark:text-gray-100">{order.Tracking_ID}</span>
                                <a
                                  href={order.Tracking_ID.startsWith("http") ? order.Tracking_ID : `https://www.ship24.com/tracking/${order.Tracking_ID}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="text-xs text-purple-600 dark:text-purple-400 underline font-semibold"
                                >
                                  Track here
                                </a>
                                <button
                                  onClick={() => {
                                    setEditingTracking(order.order_id);
                                    setCustomTracking(order.Tracking_ID || "");
                                  }}
                                  className="ml-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-1"
                                >
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536M9 13h3l9-9a1.414 1.414 0 00-2-2l-9 9v3z" />
                                  </svg>
                                </button>
                                {isUpdatingTracking && (
                          ...
                          {!order.Tracking_ID && editingTracking !== order.order_id && (
                            <button
                              onClick={() => {
                                setEditingTracking(order.order_id);
                                setCustomTracking("");
                              }}
                              className="mt-1 text-xs text-purple-600 dark:text-purple-400 underline font-semibold"
                            >
                              Add Tracking ID
                            </button>
                          )}
