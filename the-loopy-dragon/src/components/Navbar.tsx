...
export default function Navbar() {
  ...
  const [menuOpen, setMenuOpen] = useState(false); // mobile/tablet menu
  ...
  useEffect(() => {
    ...
    const updatePendingCount = () => {
      const count = Number(localStorage.getItem("pendingPaymentCount") || "0");
  ...
  // Click-away listener for user dropdown (desktop only)
  useEffect(() => {
    if (userDropdownOpen) {
  ...
  return (
    <>
      ...
      <nav
        ref={navRef}
        className="fixed left-0 right-0 z-50 border-b border-gray-100"
        style={{
          background: "#F5F9FF",
      ...
      >
        ...
        <div className="hidden lg:grid items-center h-full gap-4" style={{ gridTemplateColumns: '1fr 3fr 0.8fr' }}>
            ...
            <div className="flex items-center space-x-6 xl:space-x-8">
              {[
                ...
                <Link
                  key={item.href}
                  href={item.href}
                  className={`${montserrat.className} text-gray-700 transition-colors hover:text-gray-900 whitespace-nowrap relative`}
                  style={{
                ...
                >
                  {item.label}
                  {/* Pending Payment Badge for "Your Orders" */}
                  {item.href === "/profile" && pendingPaymentCount > 0 && (
                    <span
                      className="absolute -top-2 -right-4 rounded-full flex items-center justify-center font-bold shadow-lg border-2 border-white"
                      style={{
                        minWidth: '1.1rem',
                        height: '1.1rem',
                        fontSize: '0.7rem',
                        padding: '0 0.2rem',
                        zIndex: 2,
                        background: '#D8B6FA', // match cart notification color
                        color: '#000'
                      }}
                    >
                      {pendingPaymentCount}
          ...
          <div className="flex items-center justify-start space-x-2 pl-2">
            ...
            >
              ...
              {cartItemCount > 0 && (
                <span
                  className="absolute rounded-full flex items-center justify-center font-bold shadow-lg border-2 border-white"
                  style={{
                    top: '-0.5rem',
                    right: '-0.5rem',
        ...
        <div className="hidden md:flex lg:hidden items-center justify-between h-full px-2">
          ...
          <div className="flex items-center space-x-3 pr-6">
            ...
            <button 
              ...
              className="p-2.5 hover:bg-gray-50 rounded-xl transition-all duration-150 flex items-center justify-center relative"
              style={{
                minWidth: '2.5rem',
                minHeight: '2.5rem'
              }}
            ...
            >
              ...
              {/* Notification badge on hamburger icon for pending payments */}
              {pendingPaymentCount > 0 && (
                <span
                  className="absolute rounded-full flex items-center justify-center font-bold shadow-lg border-2 border-white"
                  style={{
                    top: '-0.375rem', // match cart icon
                    right: '-0.375rem', // match cart icon
                    minWidth: '1.25rem', // match cart icon
                    height: '1.25rem',   // match cart icon
                    fontSize: '0.75rem', // match cart icon
                    padding: '0 0.25rem', // match cart icon
                    zIndex: 2,
                    background: '#D8B6FA',
                    color: '#000'
                  }}
        ...
        <div className="flex md:hidden items-center justify-between h-full px-1 sm:px-2">
          ...
          <div className="flex items-center space-x-1 sm:space-x-2 pr-4 sm:pr-6">
            ...
            <button 
              onClick={() => setMenuOpen(!menuOpen)} 
              className="p-2 sm:p-2.5 hover:bg-gray-50 rounded-xl transition-all duration-150 touch-manipulation active:scale-95 flex items-center justify-center relative"
              style={{
                minWidth: '2.25rem',
                minHeight: '2.25rem'
              }}
            ...
            >
              ...
              {pendingPaymentCount > 0 && (
                <span
                  className="absolute rounded-full flex items-center justify-center font-bold shadow-lg border-2 border-white"
                  style={{
                    top: '-0.25rem', // match mobile cart icon
                    right: '-0.25rem', // match mobile cart icon
                    minWidth: '1.125rem', // match mobile cart icon
                    height: '1.125rem',   // match mobile cart icon
                    fontSize: '0.7rem',   // match mobile cart icon
                    padding: '0 0.2rem',  // match mobile cart icon
                    zIndex: 2,
                    background: '#D8B6FA',
                    color: '#000'
                  }}
                  ...
                  */}
                  {[
                    ...
                    <Link 
                      key={item.href}
                      href={item.href} 
                      className={`${montserrat.className} block py-3 px-4 transition-all duration-150 font-medium text-base touch-manipulation active:scale-[0.98] ${
                        // Highlight "Shop" if pathname is "/shop" or "/mobile-shop"
                        item.href === "/shop"
                          ? (pathname === "/shop" || pathname === "/mobile-shop"
                              ? 'bg-gradient-to-r from-purple-50 to-purple-100 text-purple-800 font-semibold border border-purple-200'
                              : 'rounded-xl text-gray-700 hover:bg-gray-50 hover:text-gray-900')
                          : (pathname === item.href
                              ? 'bg-gradient-to-r from-purple-50 to-purple-100 text-purple-800 font-semibold border border-purple-200'
                              : 'rounded-xl text-gray-700 hover:bg-gray-50 hover:text-gray-900')
                    ...
                    >
                      {/* Move badge beside "Your Orders" text */}
                      {item.label}
                      {item.href === "/profile" && pendingPaymentCount > 0 && (
                        <span
                          className="ml-2 rounded-full flex items-center justify-center font-bold shadow-lg border-2 border-white"
                          style={{
                            minWidth: '1.1rem',
                            height: '1.1rem',
                            fontSize: '0.7rem',
